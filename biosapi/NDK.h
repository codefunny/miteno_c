...
/**
 *@brief        设置打印起始点阵坐标(针打暂不支持)
 *@details     	在混合打印时，必须关闭边送边打，在设置打印起始坐标成功时，打印接口NDK_PrnImage的第三个参数无效。
 				混合打印流程:打印图片-->设置打印起始点阵坐标-->打印字体 或者打印字体-->设置打印起始点阵坐标-->打印图片。
 *@param        unXpos  点阵横坐标(像素)，必须满足0=<unXpos<ndk_PR_MAXLINEWIDE（正常模式为384，横向放大时为384/2）
 *@param        unYpos  点阵纵坐标(像素)
 *@return
 *@li	NDK_OK				操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		操作失败(参数错误，点阵横坐标大于ndk_PR_MAXLINEWIDE=384)
 *@li	\ref NDK_ERR_INIT_CONFIG "NDK_ERR_INIT_CONFIG" 		初始化配置失败(打印未初始化)
 *@li	\ref NDK_ERR_NOT_SUPPORT "NDK_ERR_NOT_SUPPORT" 		模式不支持(开启边送边打不支持)
 */
int NDK_PrnSetDotLine(uint unXpos,uint unYpos);
 ...
 *@return
 *@li	NDK_OK				操作成功
 *@li	\ref NDK_ERR_MODEM_RESETFAIL "NDK_ERR_MODEM_RESETFAIL"		复位失败
*/
 ...
 *@li	NDK_ERR_MI_NOTAGERR "NDK_ERR_MI_NOTAGERR" 	非接触卡-无卡,	             0xff
 *@li	NDK_ERR_MI_CRCERR "NDK_ERR_MI_CRCERR" 	非接触卡-CRC错,				     0xfe
 *@li	NDK_ERR_MI_PARITYERR "NDK_ERR_MI_PARITYERR" 	非接触卡-奇偶错,	     0xfb
 *@li	NDK_ERR_MI_BITCOUNTERR "NDK_ERR_MI_BITCOUNTERR" 	非接触卡-接收BIT错	 0xf5
 *@li	NDK_ERR_MI_BYTECOUNTERR "NDK_ERR_MI_BYTECOUNTERR" 	非接触卡-接收字节错	 0xf4
 *@li	NDK_ERR_MI_WriteFifo "NDK_ERR_MI_WriteFifo" 	非接触卡-FIFO写错误		 0xf3
 *@li	NDK_ERR_MI_ACCESSTIMEOUT "NDK_ERR_MI_ACCESSTIMEOUT" 非接触卡-接收超时	 0xe5
*/
