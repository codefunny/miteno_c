...
static int bmp_Readheader(int fid, BMPImage *bmp);
static long GetImageInfo(int fid, long lOffset, int iNumberOfChars);
static int bmp_readline(int fid, char *buf, BMPImage *bmp);
static int sip_CreateImage(unsigned short width,unsigned short height,unsigned short depth, SIPImage *imgtmp);
static unsigned short sip_getlinebytes(unsigned short len,unsigned short depth,unsigned short pad);
static int setsize(int width,int height,int pixel, PBMFILE *pbm);
static int checksize(PBMFILE *pbm);
static char *sip_getimgptr(SIPImage *image, unsigned short y);
static int writeline(int line, void *buf, PBMFILE *pbm);
static int seekto(int line, PBMFILE *pbm);
static int pbm_open(char *path, char *type, PBMFILE *pbm);
static int pbm_close(PBMFILE *pbm);
static int pbm_read_header(PBMFILE *pbm);
static int pbm_write_header(PBMFILE *pbm);
static int sip_DestroyImage(SIPImage *image);
...
int PubBmp2Pbm(uchar *pszAppFileName, uchar *pszOutFileName)
{
	char	*p;
	int	x, y;
	int	fd, iRet;
	...
	//打开文件
	fd = NDK_FsOpen((char *)pszAppFileName, "w+");	
	if (fd < 0)
	{
		return fd;
	}
...
int bmp_Readheader(int fid, BMPImage *bmp)
{
	...
	//文件定位
	iRet = NDK_FsSeek(fid, 0L, SEEK_SET);
	if(iRet <0) 
	{
		NDK_FsClose(fid);
		return iRet;
	...
	NDK_FsSeek(fid, bmp->bfOffBits, SEEK_SET);
	return 0;
}
...
long GetImageInfo(int fid, long lOffset, int iNumberOfChars)
{
	...
	iRet = NDK_FsSeek(fid, lOffset, SEEK_SET);
	if(iRet <0) 
	{
		NDK_FsClose(fid);
		return iRet;
	...
	{
		iRet = NDK_FsRead(fid, (char *)pTemp, 1);
		if(iRet <0) 
		{
			NDK_FsClose(fid);
			return iRet;
...
int bmp_readline(int fid, char *buf, BMPImage *bmp)
{
	...
	if(NDK_FsRead(fid, (char *)Tempbuf, bmp->bmp_bytesperline) <= 0)
	{
		NDK_FsClose(fid);
		return(-5);
	}
...
int checksize(PBMFILE *pbm)
{
	...
	{
		PubDebug("[checkszie][111]");
		return -1;
	}

	if ((unsigned int)pbm->height > HeightLIMIT) 
	{
		PubDebug("[checkszie][222]");
		return -1;
	}
...
int writeline(int line, void *buf, PBMFILE *pbm)
{
	...
	if(NDK_FsWrite(pbm->fid, buf, pbm->flbytes) < 0)
	{
		NDK_FsClose(pbm->fid);
		return(-6);
	}
...
//	int	errflag = 0;
	...
	iRet = NDK_FsSeek(pbm->fid, 0L, SEEK_CUR);
	if (iRet)
	{
		return iRet;
	}
	...
	iRet = NDK_FsRead(pbm->fid, buf, 79);
	if (iRet < 0)
	{
		NDK_FsClose(pbm->fid);
		return iRet;
...
int pbm_write_header(PBMFILE *pbm)
{
	...
	{
	    case 0:
			sprintf((char *)ucTempBuf, "P4\n%d %d\n", pbm->width, pbm->height);
			iRet = NDK_FsWrite(pbm->fid, (char *)ucTempBuf, strlen((char *)ucTempBuf));
			if (iRet < 0)
			{
				NDK_FsClose(pbm->fid);
				return iRet;
